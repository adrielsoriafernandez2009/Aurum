    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] rounded-2xl bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border-white/20 dark:border-zinc-800/50">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit ? 'Modifica el límite o la categoría del presupuesto.' : 'Establece un límite de gasto para controlar tus finanzas.'}
          </DialogDescription>
        </DialogHeader>
        <form action={action} className="space-y-4 pt-4">
          <div className="space-y-2">
            <Label htmlFor="categoryId">Categoría</Label>
            <select id="categoryId" name="categoryId" required defaultValue={budget?.categoryId || ""} className="flex h-10 w-full rounded-xl border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
          ...
          <div className="space-y-2">
            ...
            <Input id="amount" name="amount" type="number" step="0.01" required defaultValue={budget?.amount} placeholder="Ej: 300" className="rounded-xl" />
          </div>
